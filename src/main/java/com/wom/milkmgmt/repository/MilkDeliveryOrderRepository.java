...
public interface MilkDeliveryOrderRepository
    ...
    // find all orders by order date
    List<MilkDeliveryOrder> findByOrderDate(LocalDate orderDate);

    List<MilkDeliveryOrder> findByDeliveryPersonIdAndOrderDate(Long deliveryPersonId, LocalDate orderDate);

    // delete all orders for a delivery person on a specific date
    @Modifying
    @Transactional
    void deleteByDeliveryPersonIdAndOrderDate(Long deliveryPersonId, LocalDate orderDate);
